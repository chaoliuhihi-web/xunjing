...
const previewComponent = fs.readFileSync(path.join(root, 'components', 'xicheng', 'XichengPdfPrintPreview.vue'), 'utf8')
...
for (const token of [
  ...
  assert.ok(previewComponent.includes(token), `PDF print pagination should use unified vector button token: ${token}`)
}
