...
for (const required of [
  ...
  assert.ok(scanResult.includes(required), `Vision Agent scene-domain understanding should include ${required}`)
}
...
assert.match(
  scanResult,
  /sceneUnderstandingCards\(\)[\s\S]*domainKey:\s*'architecture'[\s\S]*domainKey:\s*'artifact'[\s\S]*domainKey:\s*'menu'[\s\S]*domainKey:\s*'food'[\s\S]*domainKey:\s*'sign-ocr'[\s\S]*domainKey:\s*'heritage'[\s\S]*domainKey:\s*'plant'[\s\S]*domainKey:\s*'animal'[\s\S]*domainKey:\s*'person'[\s\S]*domainKey:\s*'event'/,
  'Scene-domain cards should cover the product-required Vision Agent domains'
)

assert.match(
  scanResult,
  /inferSceneUnderstandingDomainScore\(card = \{\}\)[\s\S]*visionCaption[\s\S]*ocrText[\s\S]*serviceText[\s\S]*activityText[\s\S]*knowledgeGraphText/,
  'Scene-domain ranking should use camera/OCR, service, activity, and knowledge graph signals'
)

assert.match(
  scanResult,
  /createSceneUnderstandingPrompt\(card = \{\}\)[\s\S]*menu[\s\S]*辣度[\s\S]*清真[\s\S]*推荐菜[\s\S]*sign-ocr[\s\S]*翻译[\s\S]*发音[\s\S]*导航[\s\S]*heritage[\s\S]*非遗[\s\S]*体验/,
  'Scene-domain prompts should switch from generic landmark explanation to domain-specific questions'
)
