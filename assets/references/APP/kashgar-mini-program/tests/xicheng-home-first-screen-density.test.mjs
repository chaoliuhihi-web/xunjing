...
assert.match(
  style,
  /\.xicheng-home-first-screen-density \.home-action-duo\s*\{[\s\S]*margin-top:\s*12rpx/,
  'Home first-screen density layer should pull the scan/recent row closer to the hero'
)

assert.match(
  style,
  /\.xicheng-home-first-screen-density \.home-light-entry-grid\s*\{[\s\S]*margin-top:\s*12rpx/,
  'Home first-screen density layer should avoid pushing the map/travelogue row too far below the fold'
)

assert.match(
  style,
  /\.xicheng-home-first-screen-density \.home-route-recommendation-section\s*\{[\s\S]*margin-top:\s*8rpx/,
