...
public class AiApiKeyServiceImpl implements AiApiKeyService {
    ...
    public AiApiKeyDO getApiKey(Long id) {
        return AiRuntimeTenantRouter.executeForRuntimeTenant(() -> apiKeyMapper.selectById(id));
    }

    @Override
    public AiApiKeyDO validateApiKey(Long id) {
        AiApiKeyDO apiKey = getApiKey(id);
        if (apiKey == null) {
            throw exception(API_KEY_NOT_EXISTS);
        }
    ...
    public AiApiKeyDO getRequiredDefaultApiKey(String platform, Integer status) {
        AiApiKeyDO apiKey = AiRuntimeTenantRouter.executeForRuntimeTenant(
                () -> apiKeyMapper.selectFirstByPlatformAndStatus(platform, status));
        if (apiKey == null) {
            throw exception(API_KEY_NOT_EXISTS);
        }
        return apiKey;
