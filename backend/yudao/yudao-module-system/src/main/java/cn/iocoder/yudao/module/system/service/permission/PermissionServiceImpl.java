...
public class PermissionServiceImpl implements PermissionService {
    ...
    public boolean hasAnyPermissions(Long userId, String... permissions) {
        ...
        if (hasAnyTenantAdminRoleByRoles(roles)) {
            return true;
        }
    ...
    public Set<Long> getRoleMenuListByRoleId(Collection<Long> roleIds) {
        ...
        // 如果是管理员的情况下，获取全部菜单编号
        if (roleService.hasAnySuperAdmin(roleIds) || hasAnyTenantAdminRole(roleIds)) {
    ...
    private boolean hasAnyTenantAdminRole(Collection<Long> roleIds) {
        if (CollUtil.isEmpty(roleIds)) {
            return false;
        }
        return hasAnyTenantAdminRoleByRoles(roleService.getRoleListFromCache(roleIds));
    }

    private boolean hasAnyTenantAdminRoleByRoles(Collection<RoleDO> roles) {
        if (CollUtil.isEmpty(roles)) {
            return false;
        }
        return roles.stream()
                .anyMatch(role -> role != null && Objects.equals(role.getCode(), RoleCodeEnum.TENANT_ADMIN.getCode()));
    }
