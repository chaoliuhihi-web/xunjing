...
public class TenantServiceImpl implements TenantService {
    ...
    private Long createRole(TenantPackageDO tenantPackage) {
        ...
        // 分配权限
        permissionService.assignRoleMenu(roleId, resolveTenantAdminMenuIds(tenantPackage.getMenuIds()));
        return roleId;
    }
    ...
    public void updateTenant(TenantSaveReqVO updateReqVO) {
        ...
        if (isSystemTenant(tenant)) {
            // 系统租户只允许维护基础展示信息，关键治理字段保持固定
            updateObj.setPackageId(TenantDO.PACKAGE_ID_SYSTEM);
            updateObj.setStatus(tenant.getStatus());
            updateObj.setExpireTime(tenant.getExpireTime());
            updateObj.setAccountCount(tenant.getAccountCount());
    ...
    public void updateTenantRoleMenu(Long tenantId, Set<Long> menuIds) {
        TenantUtils.execute(tenantId, () -> {
            ...
            Set<Long> tenantAdminMenuIds = resolveTenantAdminMenuIds(menuIds);
            ...
            roles.forEach(role -> {
                ...
                if (Objects.equals(role.getCode(), RoleCodeEnum.TENANT_ADMIN.getCode())) {
                    permissionService.assignRoleMenu(role.getId(), tenantAdminMenuIds);
                    log.info("[updateTenantRoleMenu][租户管理员({}/{}) 的权限修改为({})]", role.getId(), role.getTenantId(), tenantAdminMenuIds);
                    return;
                }
                // 如果是其他角色，则去掉超过套餐的权限
    ...
    public void deleteTenantList(List<Long> ids) {
        // 1. 校验存在
        ids.forEach(this::validateDeleteTenant);
    ...
    private TenantDO validateDeleteTenant(Long id) {
        TenantDO tenant = validateTenantExists(id);
        if (isSystemTenant(tenant)) {
            throw exception(TENANT_CAN_NOT_DELETE_SYSTEM);
        }
        return tenant;
    }
    ...
    private Set<Long> resolveTenantAdminMenuIds(Set<Long> fallbackMenuIds) {
        Set<Long> allMenuIds = CollectionUtils.convertSet(menuService.getMenuList(), MenuDO::getId);
        return CollUtil.isNotEmpty(allMenuIds) ? allMenuIds : CollUtil.emptyIfNull(fallbackMenuIds);
    }
